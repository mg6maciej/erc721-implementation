...
const ERC721 = artifacts.require("ZeroXCertERC721.sol");
...
contract("ZeroXCertERC721", ([owner, alice, bob, charlie]) => {

    beforeEach(async () => {
        this.erc721 = await ERC721.new();
    });

    it("Alice has zero balance after burn", async () => {
    ...
    it("Alice's balance is decremented after burn", async () => {
        await this.erc721.mint(alice);
        await this.erc721.mint(alice);
        await this.erc721.burn(0);
        const balance = await this.erc721.balanceOf(alice);
    ...
    it("Alice has zero balance after burning all tokens", async () => {
        await this.erc721.mint(alice);
        await this.erc721.mint(alice);
        await this.erc721.mint(alice);
        await this.erc721.burn(0);
        await this.erc721.burn(1);
        await this.erc721.burn(2);
        const balance = await this.erc721.balanceOf(alice);
        assert.strictEqual(balance.toNumber(), 0);
    });
    ...
    it("Total supply is decreased after burn", async () => {
        await this.erc721.mint(alice);
        await this.erc721.mint(alice);
        await this.erc721.burn(0);
        const supply = await this.erc721.totalSupply();
    ...
    it("Throws on burn of nonexistent token", async () => {
